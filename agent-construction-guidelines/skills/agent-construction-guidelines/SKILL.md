...
These guidelines are derived from a multi-source qualitative synthesis of popular open-source LLM agent frameworks, framework APIs and source-level mechanisms, and public engineering/security guidance. They are not copied from a single framework, vendor, or product manual.
...
- translating research best practices into developer-facing checks
...
3. public engineering and security guidance from sources such as OWASP, OpenAI, Anthropic/Claude, Cursor, GitHub, and Composio.
...
The practices are recurring engineering concerns, not a mutually exclusive taxonomy.
...
| BP | Guideline | RQ3 Detectable |
|---|---|---|
| BP1 | Design memory for the task, not just chat history | Yes |
| BP2 | Give every agent a clear operating description | Yes |
| BP3 | Bound execution and make recovery explicit | Yes |
| BP4 | Keep tools atomic and toolsets small | Yes |
| BP5 | Write tool descriptions as model-facing instructions | Yes |
| BP6 | Keep credentials out of source code | Yes |
| BP7 | Make agent execution observable | Yes |
| BP8 | Name tools as stable agent-facing interfaces | No |
| BP9 | Separate state from memory and manage it explicitly | No |
| BP10 | Keep the agent system as simple as the task allows | No |
| BP11 | Use LLMs only where judgment is needed | No |
| BP12 | Break complex tasks into small executable steps | Not stable |
| BP13 | Select models according to task requirements | No |
| BP14 | Plan before tool use, coding, or orchestration | No |
...
### Downstream Defect Clues

Detectable in LangChain/LangGraph when implementations over-rely on trimming or deletion while rarely using summarization or other information-retention strategies.
...
Detectable defects include missing goal, missing backstory or context, missing success criteria, missing example, and semantic mismatch among description components.
...
Detectable defects include missing iteration limits, missing retry/recovery, missing termination control, and missing checkpointing.
...
Detectable defects include excessive tool count and overlapping tool functions. A conservative screening threshold is 20 or more tools exposed to a single agent.
...
Detectable defects include missing explicit input/output specification and overly long descriptions. A practical heuristic threshold is 600 characters.
...
Detectable defects include hard-coded credentials and exposed API keys, tokens, or secrets.
...
Detectable defect: absence of monitoring, tracing, or logging mechanisms.
...
Not currently operationalized as a downstream construction defect because stable repository-level detection is ambiguous.
...
Not currently operationalized as a downstream construction defect because state design is highly framework- and application-specific.
...
Not currently operationalized as a downstream construction defect.
...
Not currently operationalized as a downstream construction defect because intent and task requirements are hard to infer reliably from code alone.
...
Not currently stable enough for downstream defect detection because appropriate granularity depends on the task.
...
Not currently operationalized as a downstream construction defect because model fit cannot be judged reliably without task and evaluation context.
...
Not currently operationalized as a downstream construction defect because repository-level planning evidence is often implicit or task-dependent.

## Detectable Construction Defects

A construction defect is an observable violation of an agent-construction best practice that may weaken robustness, maintainability, security, observability, or controllability. It is not necessarily an observed runtime failure.

Currently operationalized downstream defect categories:
...
The remaining practices are still useful engineering guidelines, but are not currently operationalized as stable downstream defects because repository-level evidence is ambiguous, framework-specific, or hard to distinguish from legitimate design choices.
